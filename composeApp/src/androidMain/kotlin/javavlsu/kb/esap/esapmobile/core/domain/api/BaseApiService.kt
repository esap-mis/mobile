...
abstract class BaseApiService(
    ...
    ): javavlsu.kb.esap.esapmobile.core.domain.api.ApiResponse<T> {
        return try {
            val response = client.request { block() }
            _root_ide_package_.javavlsu.kb.esap.esapmobile.core.domain.api.ApiResponse.Success(response.body())
        } catch (e: Exception) {
            handleException(e)
        }
    }

    protected suspend fun <T> handleException(e: Exception): javavlsu.kb.esap.esapmobile.core.domain.api.ApiResponse<T> {
        return when (e) {
